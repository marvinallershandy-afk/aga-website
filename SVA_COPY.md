# SVA_COPY — Die Stimme des Vereins (v4, Etappe 1)

**Ton:** Dorfverein mit Herz und Selbstironie, stolz seit 1949. Konkret statt
Floskel, warm statt Werbung, jeder Abschnitt endet mit einem nächsten Schritt.
Keine erfundenen Fakten oder Namen — was nicht belegbar ist, bleibt bewusst
allgemein-warm.

---

## Hero / Verein
- **Kicker:** `Seit 1949 · Agathenburg`
- **Titel:** `WILLKOMMEN AM PLATZ`
- **Body:** Ein Dorf, ein Flutlicht, elf Mann — und eine Bande, hinter der man
  jeden kennt. Der SV Agathenburg-Dollern ist seit 1949 das Wohnzimmer von
  Agathenburg. Scroll dich einmal quer über unseren Platz und schau, ob du
  bleibst. Spoiler: Du bleibst.
- **Claim (rot):** Ein Dorf. Ein Verein. Ein Platz.
- **Nächster Schritt:** Scroll-Hint „Anstoß per Scroll"

## Mannschaft
- **Kicker:** `1. Herren`
- **Titel:** `UNSERE MANNSCHAFT`
- **Body:** Sechzehn Typen, die montags humpeln und sonntags fliegen. Jeder von
  ihnen als Sammelkarte — mit Holo-Glanz, echten Zahlen und genug Stolz für ein
  ganzes Album. Tipp eine Karte an und teil deinen Spieler in der Story.
- **Nächster Schritt:** Karten antippen → Modal → Story-Share.

## Tabelle
- **Kicker:** `Ergebnisse & Tabelle`
- **Titel:** `DIE WAHRHEIT VOM WOCHENENDE`
- **Body:** Manchmal Tabellenführer der Herzen, manchmal einfach Tabellenführer.
  Hier steht, was am Wochenende wirklich passiert ist — live von fussball.de,
  ungefiltert und gelegentlich glorreich.
- **Nächster Schritt:** Button „Auf ein Bier? Reinkommen" (Partyraum) bleibt an
  dieser Station.

## Mitmachen (Sektion „kontakt")
- **Kicker:** `Mitmachen`
- **Titel:** `KOMM VORBEI`
- **Body:** Probetraining ist bei uns ein kompliziertes Verfahren: hinkommen,
  Schuhe an, fertig. Dienstag & Donnerstag ab 19 Uhr [PLATZHALTER], kein Anruf
  nötig, keine Ausrede möglich.
- **Gesucht-Grid (3 Bausteine, je mit CTA):**
  1. **SPIELER** — Du kannst kicken? Oder glaubst es zumindest? Beides reicht
     für den Anfang. → `Probetraining: einfach da sein` (mailto)
  2. **HELFER & FANS** — Bande streichen, Grill anwerfen, laut sein: Ein Verein
     lebt von Leuten, die einfach da sind. Sonntag ist Heimspiel — du bist
     eingeladen. → `Sonntag vorbeikommen` (Anfahrt/Adresse)
  3. **SPONSOREN** — Deine Bande wartet schon, wir haben sie extra freigelassen.
     Logo am Platz, Reichweite im Dorf, Herz inklusive. → `Bande sichern` (mailto)
- **Footer:** © Seit 1949 · SV Agathenburg-Dollern e.V. · Mit Herz gebaut,
  Platzhalter ehrlich markiert.

## Musik-Sektion = Partyraum (Etappe 2, Kurskorrektur Marvin)
Die Musik-Station der Fahrt IST der Partyraum: Erreicht der Scroll die
Sektion, schneidet die Kamera per Dip-to-Black ins Vereinsheim (exakt
reversibel). Draußen gibt es nur Mute-Toggle + Stadion-Atmo — Musik lebt
ausschließlich im Raum (rein → Musik an, raus → Musik weich aus).
- **Kicker:** `Vereinsheim · Partyraum`
- **Titel:** `AGA URKNALL`
- **Body:** Willkommen im Partyraum — hier läuft unsere eigene Musik. Zehn
  Songs über Anpfiff, Abpfiff und alles, was dazwischen knallt, geschrieben
  von einem von uns. Drück Play, dreh auf — und wenn du mitgrölen kannst,
  bist du schon fast Mitglied.
- **Story (unter dem Cover-2-Artwork):** Angefangen hat es mit einem Banner
  am Zaun: AGA URKNALL, est. 2024. Daraus wurde ein Album — geschrieben,
  gesungen und abgemischt von einem von uns. Für die Kabine, den
  Mannschaftsbus und die dritte Halbzeit genau hier in diesem Raum.
- **Streaming-Slots:** `[Spotify · bald]` `[Apple Music · bald]` — Platzhalter.

## Microcopy
| Ort | Alt | Neu |
|---|---|---|
| Tor primär | Mit Ton betreten | **Mit Ton betreten** (bleibt — Klarheit schlägt Witz am Einstieg) |
| Tor sekundär | Ohne Ton | **Lieber leise** |
| Tor Laden | Lädt … % | **Rasen wird gemäht … %** |
| Tor Wiederkehrer | Willkommen zurück | **Willkommen zurück am Platz** |
| Scroll-Hint | Scroll | **Anstoß per Scroll** |
| Party rein/raus | Buttons | **entfallen** — der Raum ist die Musik-Station, Scroll/Nav „Musik" führt rein und raus |
| Share ok | Bild wurde heruntergeladen … | **Bild ist im Download-Ordner — ab damit in deine Story.** |
| Share Fehler | Teilen nicht möglich … | **Hat nicht geklappt — einmal noch, der Ball war im Aus.** |
| Modal zurück | Zurück | (bleibt) |
| Widget-Hinweis | Platzhalter-Tabelle … | (bleibt — ehrlich & sachlich) |
| Nav | verein/mannschaft/tabelle/kontakt | **Verein / Mannschaft / Tabelle / Mitmachen** |

## 10-Sekunden-Test (Hero allein)
Wer: SV Agathenburg-Dollern (Logo, Titelzeile, Kicker). Wo: Agathenburg (Kicker).
Was kann ich hier: scrollen & den Platz erleben (Scroll-Hint), mitmachen
(Nav „Mitmachen" sichtbar). → bestanden.

---

## v6 · Etappe 5 — Faktenkorrekturen (verifizierte Vereinsfakten)

| Feld | vorher | jetzt (korrekt) |
|---|---|---|
| Platz-Adresse | „Sportplatz Agathenburg, Am Sportplatz 1" (falsch) | **Waldsportplatz Agathenburg, Zur Mehrzweckhalle, 21684 Agathenburg** |
| Karten-Copy (Mannschaft) | „Holo-Glanz" (Look existiert nicht mehr) | „Metall-Foil" (entspricht v5.6-Karten) |
| Musik-Story | generisch | **Aga Urknall** = Truppe, die **2024 den Riesenkicker** holte (neben Dynamo Tresen, Durstige Männer) |
| Trainingszeit | „ab 19:00 Uhr" | ⚠️ **PLATZHALTER — Marvin bestätigen** (Kandidat Di+Do 16:30–17:30 lt. Recherche); aktuell plausibler Abendslot angezeigt |

**Neu E4 — „Wo wir kicken" (Route):** SVG-Mini-Map mit Marker auf dem Waldsportplatz +
plattform-bewusster Route-Button (iOS → Apple Karten `maps.apple.com/?daddr=…&dirflg=d`,
sonst → Google Maps `dir/?api=1&destination=…`). Beide Links mit korrektem Ziel belegt.

## Offene Fakten (Marvin)
- ⚠️ **Trainingszeit bestätigen.**
- fussball.de Team-Permanent-ID (Platzhalter in `CLUB.fussballDeTeamId`).
- Restliche ~10 Spielerfotos (Higgsfield-Credits leer / echte Fotos) — sonst Platzhalter-Karten.
